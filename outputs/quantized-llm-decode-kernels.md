...
For prefill handling, the surveyed systems point to an architectural pattern rather than a purely kernel-level fix. Orca introduced iteration-level scheduling/selective batching for generation [8]. vLLM/PagedAttention made large continuous batches practical by reducing KV-cache fragmentation [9]. Sarathi-Serve, vLLM chunked prefill, DeepSpeed-FastGen Dynamic SplitFuse, and TensorRT-LLM chunked context all split prompt/context work into bounded chunks and co-schedule it with decode work to reduce long decode stalls [10][11][12][13][14]. In the surveyed chunked-prefill systems, the principal exposed knob is a token/chunk budget: smaller chunks protect inter-token latency; larger chunks improve time-to-first-token and throughput [10][11].
...
## Scope, version, and terminology note

This review is **NVIDIA/CUDA-heavy** because the strongest accessible evidence concerns CUDA cores, Tensor Cores, CUTLASS, TensorRT-LLM, vLLM CUDA backends, NVIDIA PTX, and A100/Hopper/Blackwell-family documentation. AMD/ROCm, Intel GPU, TPU, NPU, and ASIC-specific designs are outside scope except where a paper is explicitly named. Repository and documentation claims were inspected as live, unpinned web sources on 2026-09-14; they should be treated as volatile snapshots rather than release-pinned reproduction evidence.

Glossary: **Q4/INT4** means 4-bit weight representation unless otherwise stated; **W4A16** means 4-bit weights with FP16/BF16 activations; **W4A8KV4** means 4-bit weights, 8-bit activations, and 4-bit KV cache; **GEMV** here denotes batch-1/very-small-batch matrix-vector-like decode; **GEMM** denotes batched matrix-matrix linear layers; **TTFT** is time to first token; **ITL/TPOT** are inter-token latency/time per output token; **backend fallback** means a quantized checkpoint is loaded but one or more layers dispatch to a slower or less specialized kernel.
...
Live vLLM documentation inspected on 2026-09-14 states that chunked prefill is enabled whenever possible in V1, prioritizes decodes before prefills, and uses `max_num_batched_tokens` as the key tradeoff knob: smaller values improve inter-token latency, while larger values improve time-to-first-token and throughput [11]. TensorRT-LLM calls the analogous feature chunked context, which divides input tokens into chunks and batches them with decode requests; it also documents scheduling policies such as first-come-first-served and equal-progress [13][14]. DeepSpeed-FastGen's Dynamic SplitFuse similarly decomposes long prompts and composes prompt/generation tokens to stabilize forward-pass size [12].
...
### Recurring findings among surveyed sources
...
2. **Q4 checkpoint format is not enough to predict performance.** Kernel backend, group size, zero-point mode, packing, and GPU architecture determine whether a model routes to Marlin, Machete, TensorRT-LLM, BitBLAS, or a slower fallback [15][17][18][21][22].
...
4. **Chunked prefill is a prominent single-GPU scheduling answer in the surveyed systems to prefill-decode interference.** Sarathi-Serve, vLLM, TensorRT-LLM, and DeepSpeed-FastGen converge on splitting context/prompt work into bounded chunks and mixing it with decode [10][11][12][13].
...
No experiment was run in this review. All speedups above are source-reported, not reproduced. The minimal reproducible experiment set for a practitioner would be:

1. **Kernel microbenchmarks with accuracy guardrails:** before speed comparisons, fix calibration data, group size, zero-point mode, activation dtype, KV-cache precision, and acceptable perplexity/task-accuracy delta versus FP16. Then benchmark FP16, W4A16 Marlin/Machete/TensorRT, and if available W4A8/QServe-like kernels on the target GPU for M=N/K shapes matching the chosen model. Report achieved bandwidth, achieved Tensor Core utilization if available, and latency vs batch size.
2. **GEMV/GEMM crossover with matched quantization quality:** for batch sizes 1, 2, 4, 8, 16, 32, measure prepacked GEMV and GEMM paths with the same quantized checkpoint, group size, zero-point mode, and activation dtype.
...
4. **Backend-selection audit:** log which kernel backend actually executes for each layer. Evidence should include framework debug logs, kernel names in Nsight Systems/Compute traces, op dispatch logs, and checkpoint metadata. This catches silent fallbacks where the checkpoint is quantized but not accelerated.
...
## Headline quantitative claims are source-reported

| Claim | Context given by source | Baseline/metric caveat |
|---|---|---|
| MARLIN reports near-ideal 4-bit speedups through moderate batch sizes and up to 2.8x end-to-end speedup in vLLM [1]. | W4A16 autoregressive linear kernels on NVIDIA GPUs; source emphasizes batch sizes around 16--32 and vLLM integration [1][2]. | Source-reported; exact speedup depends on GPU, model shape, batch, packing, and baseline kernel. |
| QServe reports prior INT4 methods can spend 20--90% runtime overhead in dequantizing weights or partial sums [3]. | W4A8KV4 serving paper motivating INT8 Tensor Core execution and optimized dequantization [3][4]. | Single-source diagnostic; applies to its evaluated prior methods and GPU setting, not every W4 kernel. |
| FLUTE reports 2--4x GEMM-kernel speedups for LUT-quantized LLMs at small batch sizes [6][7]. | Lookup-table quantized GEMM with offline restructuring and Tensor Core MMA [6][7]. | Compare only under matched quantization quality, group size, batch, and hardware; not proof of superiority over affine W4A16 generally. |
| Sarathi-Serve reports up to 2.6x higher serving capacity than vLLM for Mistral-7B on one A100 under its evaluated tail-latency constraints [10]. | Chunked prefill plus stall-free batching in the OSDI 2024 evaluation [10]. | Source-reported; baseline version/configuration and workload/SLO matter. |
...
| vLLM docs [11][21] | Live-doc snapshot of chunked-prefill policy and quantization backend matrix | Docs/version dependent |
