  ...
  "criteriaSatisfied": [
    {
      "id": "criterion-1",
      "status": "satisfied",
      "evidence": "Concrete findings are listed in the Weaknesses and Inline Annotations sections with severity labels and source file context for outputs/quantized-llm-decode-kernels.draft.md."
    }
  ],
  "changedFiles": [
    ".pi-subagents/artifacts/progress/5f827ba8/progress.md",
    ".pi-subagents/artifacts/outputs/5f827ba8/notes/quantized-llm-decode-kernels-reviewer.md"
  ],
  "testsAddedOrUpdated": [],
  "commandsRun": [
    {
      "command": "python - <<'PY' ... line-number draft view ... PY",
      "result": "failed",
      "summary": "Python was unavailable in the environment."
    },
    {
      "command": "powershell -NoProfile -Command \"$i=0; Get-Content ...\"",
      "result": "failed",
      "summary": "Initial PowerShell line-number command was incorrectly quoted through bash."
    },
    {
      "command": "powershell -NoProfile -Command '$i=0; Get-Content \"outputs/quantized-llm-decode-kernels.draft.md\" | ForEach-Object { $i++; \"{0,4}: {1}\" -f $i, $_ }'",
      "result": "passed",
      "summary": "Produced a line-numbered view of the draft for exact annotations."
    },
    {
      "command": "git status --short",
      "result": "passed",
      "summary": "Showed only untracked project/artifact directories; no staged entries."
    },
    {
      "command": "powershell -NoProfile -Command \"Test-Path '<review-artifact>'; (Get-Item '<review-artifact>').Length\"",
      "result": "passed",
      "summary": "Confirmed review artifact exists and reported nonzero length."
    },
    {
      "command": "git diff --cached --quiet; echo $?",
      "result": "passed",
      "summary": "Returned 0, confirming no staged files."
    },
    {
      "command": "powershell -NoProfile -Command '<extract acceptance-report fence and ConvertFrom-Json>'",
      "result": "passed",
      "summary": "Parsed the acceptance-report JSON successfully."
    }
  ],
  "validationOutput": [
    "Review artifact written to .pi-subagents/artifacts/outputs/5f827ba8/notes/quantized-llm-decode-kernels-reviewer.md and existence/size check passed.",
    "Progress artifact written to .pi-subagents/artifacts/progress/5f827ba8/progress.md.",
    "git diff --cached --quiet returned 0; no staged files.",
    "acceptance-json-ok"
  ],
  "residualRisks": [
    "No cited external sources were re-fetched; review is based on the draft and supplied research summary."
  ],
  "noStagedFiles": true,
  "diffSummary": "Added reviewer artifact and progress note; no source draft files edited.",
  "reviewFindings": [
    "no fatal blockers found",
    "major: outputs/quantized-llm-decode-kernels.draft.md - consensus/dominant scheduling claims overstate surveyed evidence",
    "major: outputs/quantized-llm-decode-kernels.draft.md - version-sensitive docs/repo claims lack pinned releases or access dates",
    "major: outputs/quantized-llm-decode-kernels.draft.md - source-reported numeric claims need model/GPU/baseline/metric context",
    "major: outputs/quantized-llm-decode-kernels.draft.md - broad representativeness claims such as most practical checkpoints/principal knob are under-supported",
    "major: outputs/quantized-llm-decode-kernels.draft.md - GPU scope is NVIDIA/CUDA-heavy but not explicitly limited",
    "major: outputs/quantized-llm-decode-kernels.draft.md - accuracy/perplexity constraints missing from most quantized-kernel comparisons"
  ],
  "manualNotes": "Main artifact includes structured review and inline annotations as requested."
}
```
