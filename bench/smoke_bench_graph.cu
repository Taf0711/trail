...
int main() {
    ...
    for (int i = 0; i < kWarmupLaunches; ++i) {
        increment<<<1, 1, 0, stream>>>(device_value);
    }
    check_cuda(cudaGetLastError(), "warmup kernel launches");
    check_cuda(cudaStreamSynchronize(stream), "warmup kernel execution");

    // Record exactly one sample's worth of launches (kLaunchesPerSample) into a
    // graph, once, outside the timed loop. cudaStreamCaptureModeGlobal is the
    // strictest capture mode (it errors if another thread makes untracked CUDA
    // calls during capture); fine here since this program is single-threaded.
    ...
    // Turn the recorded graph into a runnable object. This is also a one-time
    // cost, excluded from the timed loop below.
    ...
    for (int sample = 0; sample < kMeasuredSamples; ++sample) {
        ...
        // One driver submission replays all kLaunchesPerSample captured launches.
    ...
    std::printf("SM120 queued one-thread increment via CUDA Graph replay "
                "(CUDA Event microseconds/kernel)\n"
